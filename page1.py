...
def delete():        
    ...
    if delete == TRUE:
        ...
        # print(values[0])
        condition = f'''DoctorsName = "{values[0]}";'''
        conn=sqlite3.connect("hospital.db")
        cur=conn.cursor()
        sql= f'''DELETE FROM Doctors WHERE {condition} '''
        cur.execute(sql)
        conn.commit()
        conn.close()
...
def submit():
    if e1.get() == "" or e3.get() == "":
        ...
        conn=sqlite3.connect("hospital.db")
        table_query='''CREATE TABLE IF NOT EXISTS Doctors(DoctorsName TEXT, Specialization TEXT,Contact INT, Address TEXT)'''
        conn.execute(table_query)
        data_query='''INSERT INTO Doctors (DoctorsName, Specialization,Contact, Address) values (?,?,?,?)'''
        data_tuple=(
            e1.get(),
            e2.get(),
            e3.get(),
